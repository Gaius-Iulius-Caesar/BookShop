<template>
  <div style="width: 800px;margin-left: 55px;">
    <el-descriptions class="description" :column="1" :labelStyle=labelStyle :contentStyle=contentStyle border>
      <template slot="extra">
      ...
      <el-descriptions-item>
        <template slot="label"><i class="el-icon-user"></i>书店名称</template>
        {{ form.nickname }}
      </el-descriptions-item>
      <el-descriptions-item>
    ...
    <el-dialog title="书店信息" :visible.sync="dialogFormVisible" width="30%">
...
export default {
  ...
  methods: {
    load() {
      this.request.get("http://localhost:9090/user/" + this.shopuser.id).then(res => {
        if (res.code === "200") {
          this.form = res.data
        } else {
